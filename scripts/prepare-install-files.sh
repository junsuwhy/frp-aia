#!/bin/bash
# 準備安裝檔案腳本 - 將編譯好的檔案複製到 server/install-files
...
# 3. 複製二進位檔案
echo "📋 複製二進位檔案..."
cp ../cli/dist/frp-tool-linux-amd64 "$INSTALL_DIR/"
cp ../cli/dist/frp-tool-linux-arm64 "$INSTALL_DIR/"
cp ../cli/dist/frp-tool-darwin-amd64 "$INSTALL_DIR/"
cp ../cli/dist/frp-tool-darwin-arm64 "$INSTALL_DIR/"
cp ../cli/dist/frp-tool-windows-amd64.exe "$INSTALL_DIR/"
cp ../cli/dist/frp-tool-windows-arm64.exe "$INSTALL_DIR/"

# 4. 複製安裝腳本 (已在 server/install-files 中)
echo "📝 安裝腳本已在 $INSTALL_DIR/"
...
echo "  cd ../server && docker compose restart caddy"
echo ""
echo "安裝 URL："
